...
export default class PassengerRepositoryDatabase implements PassengerRepository {
	...
	async save (passenger: Passenger) {
		await this.connection.query("insert into passenger (passenger_id, name, email, document) values ($1, $2, $3, $4)", [passenger.passengerId, passenger.name, passenger.email.value, passenger.document.value]);
	}

	async get (passengerId: string): Promise<Passenger> {
		const [passengerData] = await this.connection.query("select * from passenger where passenger_id = $1", [passengerId]);
