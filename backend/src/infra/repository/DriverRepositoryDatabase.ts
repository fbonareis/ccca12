...
export default class DriverRepositoryDatabase implements DriverRepository {
	...
	async save(driver: Driver) {
		await this.connection.query("insert into driver (driver_id, name, email, document, car_plate) values ($1, $2, $3, $4, $5)", [driver.driverId, driver.name, driver.email.value, driver.document.value, driver.carPlate.value]);
	}

	async get(driverId: string) {
		const [driverData] = await this.connection.query("select * from driver where driver_id = $1", [driverId]);
